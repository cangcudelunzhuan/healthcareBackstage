...
<script>
import previewPage from '../reportManage/previewPage'
...
export default {
  ...
  computed: {
    ...
    reportPath () {
      return this.$store.getters.reportPath
    }
  },
  watch: {
    reportPath (newValue) {
  ...
  methods: {
    ...
    getTempList (pageNum, way) {
      ...
      let recycle = this.tableType === 'recycle' ? '1' : null
      this.showLoading('正在加载列表')
      reportManageApi.getCheckupList(this.numPerPage, pageIndex, this.searchParam, this.gradeStr, this.reportTypeStr, this.reportStatusStr, recycle)
      .then(res => {
        this.hideLoading()
    ...
    emptyHeight () {
      this.$nextTick(() => {
        let tableWrapHeight = getComputedStyle(this.$refs.tableWrap.$el).height
        let h = parseInt(tableWrapHeight) - 100
        if (h < 60) h = 60
        let dom = document.querySelector('.el-table__empty-block')
    ...
    // 编辑报告
    editResource (data) {
      store.dispatch('changeReportId', data.id)
      store.dispatch('changeReportPath', 'reportTypein')
      store.dispatch('changeTypeInType', 'edit')
    },
    // 删除报告
    async removeResource (data) {
      let vm = this
      await this.$store.dispatch('checkSure', {title: '你确定要删除吗?', msg: '删除后体检报告会转移至回收站，系统将保存体检报告7天之后自动删除，确认删除 ' + data.empName + ' 的体检报告吗？'})
      vm.showLoading('请稍等')
      reportManageApi.editReportStatus(data.id, data.hospitalId, '2', '')
      .then(res => {
        vm.hideLoading()
        tool.toast({
          type: 'success',
    ...
    async recoverResource (data) {
      let vm = this
      await this.$store.dispatch('checkSure', {title: '你确定要还原吗?', msg: '确认还原' + data.empName + ' 的体检报告吗？'})
      vm.showLoading('请稍等')
      reportManageApi.editReportStatus(data.id, data.hospitalId, '0', '还原')
      .then(res => {
        vm.hideLoading()
        tool.toast({
          type: 'success',
    ...
    async verifyResource () {
      ...
      await this.$store.dispatch('checkSure', {title: '你确定要发布吗?', msg: '审核发布后用户即可查看最新的体检报告，确认发布该体检报告吗？'})
      let ids = []
      let hospitalCodes = []
      ...
      reportManageApi.editReportStatus(ids.join(','), hospitalCodes.join(','), '1', '')
      .then(res => {
        this.hideLoading()
        tool.toast({
          type: 'success',
    ...
    clickRowHandle (row, event, column) {
      store.dispatch('changeReportId', row.id)
